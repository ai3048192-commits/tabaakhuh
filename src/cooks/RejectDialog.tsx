 ...
 * transient failure (FR-020).
 */
export default function RejectDialog({ storeName, busy, onSubmit, onCancel }: Props) {
  ...
  return (
    <DialogShell label={M.rejectTitle(storeName)} onDismiss={onCancel}>
      ...
      <div className="mt-1 flex items-center justify-between">
        <p
          id="reject-required"
          className={`text-xs text-red-600 ${trimmedLen === 0 ? '' : 'invisible'}`}
        >
          {M.rejectReasonRequired}
        </p>
