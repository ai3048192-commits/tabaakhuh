import { describe, it, expect, beforeEach, afterEach, vi } from 'vitest'
import { screen } from '@testing-library/react'
import { renderAtDrivers } from '../helpers/harness'
import { installFetchMock, type FetchMock } from '../helpers/fetchMock'
import { ok, fail, cityList } from '../helpers/fixtures'
import { STORAGE_KEYS } from '../../src/auth/authStorage'

let fm: FetchMock

beforeEach(() => {
  fm = installFetchMock()
})
afterEach(() => {
  vi.unstubAllGlobals()
  localStorage.clear()
})

describe('Driver review — session & access (FR-028 / FR-029)', () => {
  it('FR-028: a 401 on the queue request ends the session and redirects to /login', async () => {
    fm.reply('GET /admin/cities', { json: ok(cityList()) })
    fm.reply('GET /admin/drivers/pending', { status: 401, json: fail('Unauthenticated.') })

    renderAtDrivers(fm)

    expect(await screen.findByText('صفحة تسجيل الدخول')).toBeInTheDocument()
    expect(localStorage.getItem(STORAGE_KEYS.token)).toBeNull()
    expect(localStorage.getItem(STORAGE_KEYS.profile)).toBeNull()
  })

  it('FR-029: a signed-in non-admin is denied the area', async () => {
    fm.reply('GET /admin/cities', { json: ok(cityList()) })
    fm.reply('GET /admin/drivers/pending', { json: ok([]) })

    renderAtDrivers(fm, { admin: false })

    expect(await screen.findByText('صفحة تسجيل الدخول')).toBeInTheDocument()
    expect(screen.queryByText('طلبات السائقين')).toBeNull()
  })
})
